...
def main():
    ...
    for SYMBOL in SYMBOLS:
        ...
        for b in range(total_batches):
            ...
            win_pnl = pf_trades.winning.pnl.sum()
            loss_pnl = pf_trades.losing.pnl.sum().abs()
            pf_pf = np.where(loss_pnl == 0, np.where(win_pnl > 0, 999.0, 0.0), win_pnl / loss_pnl)
            pf_pf = pd.Series(pf_pf, index=chunk_index).fillna(0.0).round(2)
